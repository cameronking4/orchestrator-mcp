...
export class TaskAdherenceMonitor {
  checkAlignment(
    ...
    const suggestion = aligned ? undefined : this.generateSuggestion(
      planGoal,
      currentAction,
      driftScore
    );
  ...
  private checkGoalTerms(goalText: string, actionText: string): number {
    ...
    const matches = goalWords.filter(gw => 
      actionWords.some(aw => aw.includes(gw) || gw.includes(aw))
    ).length;
    
    return goalWords.length > 0 ? matches / goalWords.length : 0.5;
  }
  
  private detectDrift(goalText: string, actionText: string, contextText: string): number {
    let driftScore = 0;
    ...
    if (techMismatches) driftScore += 0.4;
    
    // Check if action mentions goal at all
    const mentionsGoal = goalText.split(/\s+/).some(word => 
      word.length > 4 && actionText.includes(word)
    );
  ...
  private detectTechMismatch(goalText: string, actionText: string): boolean {
    const techPairs = [
      ['rest', 'graphql'],
