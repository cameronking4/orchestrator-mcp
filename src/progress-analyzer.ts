...
import { TaskOrchestrator, Task } from './lib.js';
...
export class ProgressAnalyzer {
  ...
  analyzeProgress(planGoal?: string): ProgressAnalysis {
  ...
  private calculateStats(planState: any): {
    total: number;
    completed: number;
    inProgress: number;
    pending: number;
  ...
  private generateSummary(stats: any, planGoal?: string): string {
  ...
  private generateInsights(stats: any, planState: any): string[] {
  ...
  private identifyRisks(stats: any, planState: any): string[] {
    const risks: string[] = [];
  ...
  private generateRecommendations(stats: any, planState: any, planGoal?: string): string[] {
  ...
  private analyzeTaskTypes(planState: any): { backend: number; frontend: number; other: number } {
  ...
  private getMaxDepth(planState: any, depth: number = 0): number {
    if (!planState.subtasks || planState.subtasks.length === 0) {
      return depth;
    }
    return Math.max(...planState.subtasks.map((sub: any) => 
      this.getMaxDepth(sub, depth + 1)
    ));
  }

  private findReadyTasks(planState: any): string[] {
    const ready: string[] = [];
