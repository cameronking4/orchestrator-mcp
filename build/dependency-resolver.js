...
export class DependencyResolver {
    dependencies = new Map(); // task_id -> set of depends_on
    tasks = new Map();
    ...
    hasDependencyRelationship(taskDesc, otherDesc, keywords) {
    ...
    generateReasoning(task, dependsOn, allTasks) {
        if (dependsOn.length === 0) {
            return "No dependencies inferred from task description.";
        }
        const depTasks = dependsOn.map(id => allTasks.find(t => t.id === id)).filter(Boolean);
