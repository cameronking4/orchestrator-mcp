...
export class TaskEvaluator {
    evaluateTask(taskDescription, actualOutput, context) {
        // Normalize inputs
        ...
        const assessment = this.generateAssessment(taskDescription, actualOutput, goalAlignment, completeness, qualityScore, overallScore);
        const reasoning = this.generateReasoning(goalAlignment, completeness, qualityScore, taskDescription, actualOutput);
        const suggestions = passed ? undefined : this.generateSuggestions(goalAlignment, completeness, qualityScore, taskDescription);
        return {
            assessment,
            passed,
            reasoning,
    ...
    checkGoalAlignment(taskKeywords, outputKeywords, taskText, outputText) {
    ...
    assessQuality(output, context) {
        ...
        if (output.length > 50)
            score += 0.1;
        if (output.includes('created') || output.includes('implemented'))
            score += 0.1;
    ...
    generateAssessment(taskDescription, actualOutput, goalAlignment, completeness, qualityScore, overallScore) {
        if (overallScore >= 0.7) {
    ...
    generateReasoning(goalAlignment, completeness, qualityScore, taskDescription, actualOutput) {
        const parts = [];
    ...
    generateSuggestions(goalAlignment, completeness, qualityScore, taskDescription) {
        const suggestions = [];
