...
server.registerTool("evaluate_task", {
    ...
    try {
        const result = evaluator.evaluateTask(args.task_description, args.actual_output, args.context);
...
server.registerTool("check_alignment", {
    ...
    try {
        const result = adherenceMonitor.checkAlignment(args.plan_goal, args.current_action, args.recent_context);
...
server.registerTool("recall", {
    ...
    try {
        ...
        const output = result.results.map((r, i) => `${i + 1}. ${r.what}${r.details ? `\n   Details: ${r.details}` : ''}\n   Relevance: ${(r.relevance * 100).toFixed(0)}%`).join('\n\n');
        return {
            content: [{ type: "text", text: output }],
        };
    }
...
server.registerTool("infer_dependencies", {
    ...
    try {
        ...
        if (result.dependencies.length > 0) {
            output += result.dependencies.map(dep => `Task ${dep.task_id} depends on: ${dep.depends_on.join(', ')}\nReasoning: ${dep.reasoning}`).join('\n\n');
        }
        else {
...
server.registerTool("get_next_tasks", {
...
server.registerTool("analyze_progress", {
...
server.registerTool("check_constraints", {
    ...
    try {
        const result = constraintChecker.checkConstraints(args.constraints, args.output, args.context);
